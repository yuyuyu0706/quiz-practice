...
export function getQuestionPreview(text) {
  ...
  const sentenceEnd = cutPoints.length
    ? Math.min(...cutPoints) + 1
...
function appendInlineFormattedText(element, text) {
  ...
  while (match) {
    if (match.index > currentIndex) {
      element.appendChild(
        document.createTextNode(source.slice(currentIndex, match.index)),
      );
    }
...
function getWhyWrongEntries(whyWrong, choiceMap = {}) {
  ...
  const order = new Map(
    ["A", "B", "C", "D"].map((label, index) => [label, index]),
  );
  return entries.sort(
...
export function renderQuestion(els, data) {
  ...
  els.toggleExplanation.textContent = explanationOpen
    ? "解説を非表示"
    : "解説を表示";

  if (graded) {
    const correctLabel = choiceLabels.find(
    ...
    els.choicesForm.querySelectorAll("label").forEach((labelEl) => {
      ...
      if (labelKey === chosen && labelKey !== correctLabel)
...
export function renderResult(els, result) {
  ...
  const sectionText = Object.entries(result.sectionStats)
    .map(
      ([sec, stat]) => `S${sec}: ${Math.round((stat.ok / stat.total) * 100)}%`,
    )
    .join(" / ");
