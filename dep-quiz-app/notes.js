export function baseProgress() {
  return { seenCount: 0, correctCount: 0, wrongCount: 0, lastAnsweredAt: null, bookmark: false, noteText: '', noteUpdatedAt: null };
}
...
export function deleteNote(progress, questionId) { return saveNote(progress, questionId, ''); }

export function deleteAllNotes(progress) {
  const next = { ...progress };
  Object.entries(next).forEach(([key, value]) => {
    next[key] = { ...baseProgress(), ...value, noteText: '', note: '', noteUpdatedAt: null };
  });
  return next;
}
...
export function getAllNoteItems(questions, progress) {
  return questions.map((question) => {
    ...
    return { id: question.id, section: question.section, questionText: question.question, noteText, noteUpdatedAt: item.noteUpdatedAt ?? null };
  }).filter(Boolean).sort((a, b) => new Date(b.noteUpdatedAt ?? 0) - new Date(a.noteUpdatedAt ?? 0));
}
