...
全体のPhase構成・依存順・開始ゲート・現在有効な判断は、[開発ロードマップ正本](docs/roadmap.md)を参照してください。

### Phase F：出題バリエーション（計画定義済み・起票保留）
...
| F-3          | 解説内 UI・提示条件・アクセシビリティ  | F-2 完了後（かつ提示条件確定後） |

> 期限・開始日・初期コンテンツ対象数はゲート成立後に Phase F Milestone で確定します。
> 全体ロードマップの正本は [`docs/roadmap.md`](docs/roadmap.md) です。
