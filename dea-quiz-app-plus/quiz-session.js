...
export function createQuizSession(questions, settings, mode, progress) {
...
export function getChoiceLabels(choices) {
  ...
  return FIXED_CHOICE_LABELS.every((l) => keys.includes(l))
    ? [...FIXED_CHOICE_LABELS]
    : [...keys].sort();
}
...
export function buildSessionResult(session, questions, progress, getStoredSelectedLabel) {
  ...
  session.order.forEach((id) => {
    ...
    if (isCorrect) correctCount += 1;
      ...
      wrongItems.push(`${q.id}: ${q.question.slice(0, 50)}...`);
    }
...
export function normalizeLoadedSession(saved, options = {}) {
  ...
  const session = {
    ...
    app: saved.app ?? 'dea-quiz-app-plus',
