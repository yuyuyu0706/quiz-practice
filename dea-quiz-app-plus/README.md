...
| 必須性         | 任意                                             |
| 表示タイミング | 回答後                                           |
| 初期表示方針   | 誤答時に、選択した誤答の `whyWrong` があれば表示 |
| 将来拡張       | 全誤答理由表示も検討可                           |
...
Phase 2-3: questions.json への whyWrong 追加
...
次は Phase 2-1 として validator 拡張を先に進め、その後 `whyWrong` 表示、`questions.json` への `whyWrong` 追加、複数正答 / 5肢選択の採点・UI 対応へ分割して進めます。
