...
データ取り込みは、変換、分析、AI活用の前提を作る最初の工程です。ここでデータを正しく受け入れられないと、その後のSilver / Goldテーブル、Lakeflow Jobsでの運用、Unity Catalogによるガバナンスまで不安定になります。
...
ゴールは、Data Ingestion and Loadingで問われる代表的な判断軸を説明できるようになることです。特に batch / streaming / incremental loading、COPY INTO、Auto Loader、Lakeflow Connect、JDBC / REST、schema enforcement / schema evolution、Unity Catalog governed tablesを、単語としてではなく「どの状況で選ぶのか」と結び付けて理解します。
...
schema enforcementは、想定しない型やカラムを無制限に受け入れないための考え方です。一方でschema evolutionは、現実のデータソースで起きるカラム追加などを安全に取り込むための考え方です。どちらも、取り込み時点でデータの信頼性を守るために使います。

最終的に、取り込んだデータはUnity Catalogで管理されるテーブルに着地させることで、権限、リネージ、監査、発見性を扱いやすくなります。つまりData Ingestion and Loadingは、「データを置くこと」ではなく、「組織が安全に使えるデータ資産として受け入れること」と考えると理解しやすくなります。
...
このコード例では、`cloudFiles`でファイル到着を扱い、schemaLocationにスキーマ情報、checkpointLocationに処理済み位置などの状態を持たせています。これにより、単発の読み込みではなく、継続的な取り込みと再開を前提にできます。
...
Data Ingestion and LoadingでBronzeへ安全に受け入れたデータは、Data Transformation and ModelingでSilver / Goldへ整えます。ここでは、型変換、正規化、結合、重複排除、集計、品質チェックなどを通じて、分析やAIが使いやすい形に変換します。
