...
特に、[Git](#keyword-git)、[branch](#keyword-branch)、[pull request](#keyword-pull-request)、[dev](#keyword-dev)、[stg](#keyword-stg)、[prod](#keyword-prod)、[environment](#keyword-environment)、[Databricks Asset Bundles](#keyword-databricks-asset-bundles)、[bundle validate](#keyword-bundle-validate)、[deployment](#keyword-deployment)、[promotion](#keyword-promotion)を、データ品質と運用安全性の観点で理解します。
...
そのため、Databricks Asset BundlesやCLIを使うときも、重要なのはコマンドの暗記ではありません。**どの定義を、どの環境へ、どの検証を通して反映するか**を設計することです。
...
Databricks Asset Bundlesは、ジョブやパイプラインなどのDatabricks資源を、コードとして管理・配布するための定義です。Bundlesを使うと、ジョブ名、タスク、Notebook参照、環境ごとの設定を、手動クリックではなくレビュー可能なファイルとして扱えます。
