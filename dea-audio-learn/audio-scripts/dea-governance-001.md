...
一方で、厳しく閉じすぎれば、分析や業務改善の速度が落ちます。Governance and Securityは、全拒否か全開放かを選ぶ領域ではありません。必要な人が、必要な範囲で、必要な条件のもと安全にデータを使えるようにするための領域です。**Governance and Securityは、データ活用を止めずに、安全にスケールさせるための仕組み**です。

Databricksにおけるガバナンスは、データ活用を止めるための制限ではなく、データ活用のスピードと安全性を両立させるための仕組みとして理解します。

## 本チャプターのゴール

このチャプターでは、Databricksにおけるガバナンスとセキュリティの目的、および利用者ごとにデータアクセスを統制する判断軸を説明できるようになることを目指します。

特に、[Unity Catalog](#keyword-unity-catalog)、[managed table](#keyword-managed-table)、[external table](#keyword-external-table)、[GRANT](#keyword-grant)、[REVOKE](#keyword-revoke)、[DENY](#keyword-deny)、[row-level security](#keyword-row-level-security)、[column masking](#keyword-column-masking)、[ABAC](#keyword-abac)、[user](#keyword-user)、[group](#keyword-group)、[service principal](#keyword-service-principal)を、データ活用と統制を両立するための判断軸として理解します。
...
顧客情報、個人情報、契約情報、売上、監査ログは、用途や立場によって見せる範囲が変わります。たとえば、分析者は集計済みのGoldテーブルを見られても、基盤管理権限や個人情報の生値までは不要かもしれません。**価値の高いデータほど、利用可能性と制御の両方を設計する必要があります**。
...
もし権限管理がワークスペースやテーブルごとに分散すると、誰が何にアクセスできるかを把握しづらくなります。手作業による権限付与は、過剰権限や監査漏れにつながります。**権限は個別対応ではなく、組織として説明できるルールとして設計する**ことが重要です。
...
Unity Catalogがあることで、ワークスペースごとに散らばった権限管理ではなく、組織としてどのデータ資産があり、誰が使えるのかを把握しやすくなります。**Unity Catalogは、データ資産とアクセス制御を一元的に扱うための基盤**です。

### 権限は、最小権限と役割単位で設計する

GRANT / REVOKE / DENYは、user、group、service principalに対して権限を制御する基本です。原則として、個人に直接付与するよりも、groupやservice principalを中心に設計します。

最小権限の考え方では、利用者が目的を達成するために必要な範囲だけを許可します。分析者にはSELECTを許可しても、基盤管理やテーブル変更の権限までは不要な場合があります。**権限設計では、誰に何を許すかより先に、どの役割に何が必要かを整理します**。
...
column maskingは、メールアドレス、電話番号、個人識別子などの機密列を、必要な利用者にだけ見せるための考え方です。row filterやcolumn maskを使うことで、テーブルを複製して権限別に増やすのではなく、ポリシーで見え方を制御できます。**同じテーブルを使いながら、利用者ごとに見える範囲を変えること**が重要です。

### ABACは、属性に基づいて統制をスケールさせる

利用者やテーブルが増えるほど、個人やテーブル単位で権限を手作業管理し続けることには限界があります。部署、地域、データ分類、機密度、利用目的といった属性を使えば、同じ考え方のルールを多くのデータ資産へ一貫して適用しやすくなります。
...
データ基盤では、利用者ごとに見せるデータ範囲を変えることが重要です。全員に同じテーブルを同じ見え方で開放するのではなく、管理者、分析者、部門利用者、外部パートナーごとに、操作できる範囲や見える行・列を変えます。
...
catalog、schema、tableには階層があります。必要最小限の権限を付けるには、上位階層を使える権限と、実際に参照するテーブルへの権限を分けて考えます。次の例では、分析者グループにGoldテーブルを参照するための最小限の権限を段階的に付与しています。
...
テーブル全体を拒否すると、分析や業務利用まで止まってしまうことがあります。一方で、メールアドレスや個人情報列を全員に見せる必要はありません。こうした場合は、テーブル利用は許可しつつ、列単位で見え方を制御する考え方が有効です。
...
データ基盤では、データを取り込み、整え、運用するだけでは十分ではありません。誰がどのデータを使えるかを適切に統制して初めて、安心して利用できる基盤になります。

ここまで学んだ各領域を横断しながら、データ品質、運用、権限、再利用性を一体で考えることが、データエンジニアに求められる視点です。
