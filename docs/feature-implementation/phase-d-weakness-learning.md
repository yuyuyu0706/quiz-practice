...
問題を解いて学習履歴を保存できても、その履歴だけから次に何を学ぶべきかを判断することは難しい。弱点分析は、蓄積された履歴を次の学習判断へ変える**起点**である。
...
図3では、学習履歴を4つの観点から整理し、次の学習行動へ活用する考え方を示している。読み手がここで押さえるべき点の1つ目は、**低い正答率を機械的に同じ弱点として扱わないこと**である。未学習、データ不足、実際に誤答が残る弱点では学習上の意味が異なる。これらを区別できなければ、学習効果を高める復習にはつながらない。
