...
第3章で示したとおり、弱点分析は学習履歴を毎回参照して分析結果を導き出す。分析結果を直接クリアしても、元となる学習履歴が残れば、次の分析では同じ判断が得られる。新しい学習周期を始めるには、判断の源泉である学習履歴を区切る必要がある。

### 学習履歴と学習資産はどう区別されているか

重要なのは、**学習履歴と学習資産を区別すること**である。正誤回数・誤答理由・最終回答日時のような「現在の判断材料」はリセット対象となる。これらは現在の学習周期における評価結果であり、新しい周期を始める際には一区切りにする。

一方、利用者が育てたメモ・ブックマーク・設定は**学習資産**である。新しい学習周期でも足場となるため、再利用できるように残す。リセット後にメモもブックマークも持たないエントリは保存先から削除され、保持の意味がある記録だけが残る。

### 中断セッションはどう扱われるか

リセット計画を生成する際に、進行中の中断セッションが存在するかどうかも確認する。中断セッションが残ると、リセット後も途中から再開できてしまう状態になる。そのため中断セッションはリセット計画の対象に含め、リセットを確定するタイミングで同時に削除する。リセット後は再開ボタンが表示されず、新しい学習周期の起点から始められる。

### なぜ計画生成と実行を分離し、二段階確認が必要か

リセットは取り消せない操作である。そのためリセットの計画生成と実行を分離した二段階の設計にしている。計画生成の段階では、リセット対象の問題数・保持されるメモ件数・ブックマーク件数・中断セッションの有無を含む影響範囲を算出するが、この時点では保存先への変更は一切行わない。利用者が影響範囲を確認したうえで明示的に確定した場合にのみ、実際の保存処理が実行される。キャンセルやダイアログを閉じた場合、保存先は変更されないことが保証されている。

### 保存失敗時に中途半端な状態を残さないのはなぜか

リセットの保存処理が途中で失敗した場合、一部だけリセットされた中途半端な状態は残さない。復元に成功した場合はダイアログが開いたまま再試行できる状態が維持され、保存先への変更は生じていない。復元にも失敗した場合は再試行の操作をブロックし、再読み込みで状態を確認するよう案内する。どちらの場合も、利用者が意図しない途中状態で学習を続けることを防ぐ。

### リセット後に分析が未学習状態へ戻り、学習資産を使い続けられるか

リセット後に弱点分析を実行すると、誤答数はゼロ・正答率は未算出の状態から再出発する。弱点起点の復習では対象問題が存在しない状態になり、学習履歴が白紙の新しい周期の起点として機能する。一方、保持されたブックマークを使った復習やメモを持つ問題の復習は引き続き機能し、利用者が育てた学習資産を新しい周期でも活用できる。
