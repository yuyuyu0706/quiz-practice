...
学習履歴は、正誤の記録や解答日時といった「解いた事実」の集まりであり、そのままでは次にどこを優先して学ぶべきかという観点では整理されていない。また、正答率が低いという結果だけを見ても、まだ十分に取り組めていないのか、実際に理解が不足しているのかを区別できず、一律に同じ弱点として扱うことはできない。弱点分析が担うのは、これらの学習履歴を、次に優先して見直すべきポイントの判断材料へと変換することであり、分析結果を示すこと自体を目的にするものではない。
...
この循環は、いくつかの地点がそれぞれ異なる問いに答えることで成り立っている。弱点分析は「いま何を優先して見るべきか」を判断し、対象確認は分析結果を「実際に取り組む問題」として具体化し、復習は既存のクイズ体験でその対象に取り組み、履歴反映は結果を既存の学習履歴へ書き戻す。復習結果を専用の履歴に閉じず既存の学習履歴へ戻しているのは、更新後の履歴から弱点分析を再び実行し、次の判断を導けるようにするためである。この往復があることで、判断は一度きりで終わらず、学習が進むたびに更新され続ける。
