...
function assessmentSectionSummary(section, overrides = {}) {
  return {
    section,
    sectionTitle: `Section ${section}`,
...
test('buildWeaknessAssessment falls back invalid minAnsweredQuestionCount values to the default', () => {
  [-1, 1.5, '3', null, {}].forEach((minAnsweredQuestionCount) => {
    const assessment = buildWeaknessAssessment(
      {
        overall: { answeredQuestionCount: 3 },
        sections: [assessmentSectionSummary('1')],
        tags: [],
      },
      { minAnsweredQuestionCount }
    );
...
test('buildWeaknessAssessment prioritizes the section with the highest wrong count', () => {
  assert.equal(
    prioritySectionFor([
      assessmentSectionSummary('1', {
        wrongCount: 1,
        correctCount: 4,
        totalAttemptCount: 5,
        accuracyRate: 0.8,
      }),
      assessmentSectionSummary('2', {
        wrongCount: 3,
        correctCount: 3,
        totalAttemptCount: 6,
        accuracyRate: 0.5,
...
test('buildWeaknessAssessment uses total attempts when wrong count and accuracy tie', () => {
  assert.equal(
    prioritySectionFor([
      assessmentSectionSummary('1', {
        wrongCount: 2,
...
test('buildWeaknessAssessment preserves section order when priority comparisons tie', () => {
  assert.equal(
    prioritySectionFor([
      assessmentSectionSummary('2', {
        wrongCount: 2,
...
test('buildWeaknessAnalysis safely normalizes malformed progress inputs', () => {
  assert.doesNotThrow(() =>
    prepareWeaknessAnalysisInput([question('Q1', '1')], 'invalid-progress')
  );
  assert.doesNotThrow(() =>
    buildWeaknessAnalysis([question('Q1', '1'), question('Q2', '1'), question('Q3', '1')], {
      Q1: null,
      Q2: progressEntry({ seenCount: -1, correctCount: -2, wrongCount: -3 }),
      Q3: progressEntry({ seenCount: 'seen', correctCount: 'correct', wrongCount: 'wrong' }),
    })
  );

  const nonObjectProgressInput = prepareWeaknessAnalysisInput(
    [question('Q1', '1')],
    'invalid-progress'
  );
  assert.deepEqual(nonObjectProgressInput.questionItems[0].progress, {
    seenCount: 0,
    correctCount: 0,
    wrongCount: 0,
    lastAnsweredAt: null,
